...
//Maximal theoretical entries = (5*1024*1024)/(datatype)
...
//open file, Assuming cells is the name and it is in the same directory
		//char cwd[PATH_MAX];
		//char filename[] = "/cells";
	...
	nChunks = (nRows % 100000) ? nRows/maxRows +1 :  nRows/maxRows;
	...
	for (size_t i = 0; i< nChunks; i++){
		...
		size_t ix;
		#pragma omp parallel for private(ix,line) shared(chunk, chunkSize)
		for( ix = 0; ix < chunkSize; ix++) {
		
			fgets(line, 25, file);
			
			chunk[ix][0] = atof(line+0);
			chunk[ix][1] = atof(line+8);
			chunk[ix][2] = atof(line+16);
		}
		...
		if(i > 0){
			...
			for(size_t ix = 0; ix < prevIndex; ix++){
				...
				char line[25];
				#pragma omp parallel for private(line,prevChunkEntries) shared(chunk,chunkSize)
				for(size_t jx = 0; jx < prevIndexRow; jx++){
				
					fgets(line,25,file);
					prevChunkEntries[0] = atof(line+0);
					prevChunkEntries[1] = atof(line+8);
					prevChunkEntries[2] = atof(line+16);
